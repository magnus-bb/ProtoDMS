...
export function path(canvas, pathData) {
	...
	function select(evt) {
		...
		// to edit mode
		//* DON'T CHANGE TO OFFSET INSTEAD OF CLIENT COORDS
		if (state === 1) {
			state = 2
			settingsPnl = modalCreate(
				evt.clientX - 10,
				evt.clientY - 10,
				new PathSettings(canvas, svgGrp)
			)
			return
		}
	...
	const reset = moveEvtProc(
		...
		/** @param {PointerEvent & { target: Element} } evt */ evt => {
			...
			movedEnd = pathData.e.el.contains(evt.target)
				? 'e'
				: pathData.s.el.contains(evt.target)
				? 's'
				: null
			...
			//* KEEP AS OFFSET INSTEAD OF CLIENT COORDS
			if (pathData[movedEnd].shape) {
				if (
					pathData[movedEnd].shape.shapeEl !== pathData[movedEnd === 's' ? 'e' : 's'].shape?.shapeEl
				) {
				...
				pathData[movedEnd].data = {
					...
					position: pointInCanvas(canvas[CanvasSmbl].data, evt.offsetX, evt.offsetY),
				}
			}
...
function shapeObj(pathConnectedShape) {
	return pathConnectedShape?.shapeEl[ShapeSmbl]
}

/** @param {PathEnd} pathEnd */
function endDraw(pathEnd) {
	pathEnd.el.style.transform = `translate(${pathEnd.data.position.x}px, ${
		pathEnd.data.position.y
	}px) rotate(${arrowAngle(pathEnd.data.dir)}deg)`
}
...
function arrowAngle(dir) {
	return dir === 'right' ? 180 : dir === 'left' ? 0 : dir === 'bottom' ? 270 : 90
}

/** @param {Dir} dir, @return {Dir} */
export function dirReverse(dir) {
...
function dirByAngle(s, e) {
	...
	return numInRangeIncludeEnds(rad, -0.8, 0.8)
		...
		? 'top'
		: numInRangeIncludeEnds(rad, 2.4, 3.2) || numInRangeIncludeEnds(rad, -3.2, -2.4)
		? 'right'
		: 'bottom'
}
...
function pathCalc(data) {
	let coef =
		Math.hypot(
			...
			data.s.data.position.y - data.e.data.position.y
		) * 0.5
	...
	function cy(pathEnd) {
		return pathEnd.dir === 'right' || pathEnd.dir === 'left'
			? pathEnd.position.y
			: pathEnd.dir === 'bottom'
			? pathEnd.position.y + coef
			: pathEnd.position.y - coef
	}

	return (
		`M ${data.s.data.position.x} ${data.s.data.position.y} C ${cx(data.s.data)} ${cy(
			data.s.data
		)}, ` +
...
function childs(el, ...keys) {
	return keys.map(kk => child(el, kk))
}

/** @param {number} num, @param {number} a, @param {number} b */
function numInRangeIncludeEnds(num, a, b) {
	return a <= num && num <= b
}
