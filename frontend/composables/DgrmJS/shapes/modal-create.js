...
export function modalCreate(bottomX, bottomY, elem) {
	...
	editModalDiv.style.cssText =
		'position: fixed; box-shadow: 0px 0px 58px 2px rgb(34 60 80 / 20%); border-radius: 16px; background-color: rgba(255,255,255, .9);'
	editModalDiv.append(elem)
	document.body.append(editModalDiv)

	function position(btmX, btmY) {
		editModalDiv.style.left = `${btmX}px`
		editModalDiv.style.top = `${
			window.scrollY + btmY - editModalDiv.getBoundingClientRect().height
		}px` // window.scrollY fix IPhone keyboard
	}
