...
export function copyPastApplay(canvas) {
	listen(
		document,
		'paste',
		/** @param {ClipboardEvent & {target:HTMLElement | SVGElement}} evt */ evt => {
...
function copyDataCreate(shapes) {
	return deepCopy(serializeShapes(shapes))
}
...
export function groupSelectApplay(canvas) {
	...
	function onMove(evt) {
		...
		// draw rect
		const x = evt.offsetX - selectStart.x
		const y = evt.offsetY - selectStart.y
		selectRect.width.baseVal.value = Math.abs(x)
		selectRect.height.baseVal.value = Math.abs(y)
		if (x < 0) {
			selectRectPos.x = evt.offsetX
		}
		if (y < 0) {
			selectRectPos.y = evt.offsetY
		}
	...
	listen(
		...
		/** @param {PointerEvent} evt */ evt => {
			...
			timer = setTimeout(_ => {
				...
				positionSet(startCircle, { x: evt.offsetX, y: evt.offsetY })
				svg.append(startCircle)

				selectStart = { x: evt.offsetX, y: evt.offsetY }
				selectRectPos = { x: evt.offsetX, y: evt.offsetY }
...
function groupEvtProc(canvas, selected) {
	...
	if (
		...
		selected.pathEnds?.length === 2 &&
		selected.pathEndsPaths?.length === 1
	) {
		pathUnhighlight(selected.pathEndsPaths[0])
 ...
 */
function pointInRect(rectPosition, rectWidth, rectHeight, x, y) {
	return (
		rectPosition.x <= x &&
		x <= rectPosition.x + rectWidth &&
		rectPosition.y <= y &&
		y <= rectPosition.y + rectHeight
	)
}
