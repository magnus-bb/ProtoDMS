...
export function textareaCreate(textEl, verticalMiddle, val, onchange, onblur) {
	...
	// must be in px
	const textareaPaddingAndBorder =
		parseInt(textareaStyle.paddingLeft) + parseInt(textareaStyle.borderWidth)
	function draw() {
		return foreignWidthSet(
			textEl,
			foreign,
			textarea,
			textareaPaddingAndBorder,
			textareaStyle.textAlign
		)
	}
	draw()
...
function foreignWidthSet(textEl, foreign, textarea, textareaPaddingAndBorder, textAlign) {
	...
	foreign.x.baseVal.value =
		textBbox.x -
		textareaPaddingAndBorder -
		(textAlign === 'center' ? 10 : textAlign === 'right' ? 20 : 0)
