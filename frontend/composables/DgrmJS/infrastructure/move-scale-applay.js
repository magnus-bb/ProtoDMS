...
export const pointInCanvas = (canvasData, x, y) => {
	//* THIS WORKS WHEN X AND Y ARE OFFSETX AND OFFSETY
	return {
...
function applayFingers(svg, canvasData, scaleFn, transformFn) {
	...
	function move(evt) {
		...
		if ((firstPointer && !secondPointer) || (!firstPointer && secondPointer)) {
			...
			canvasData.position.x = evt.offsetX + (firstPointer || secondPointer).shift.x
			canvasData.position.y = evt.offsetY + (firstPointer || secondPointer).shift.y
			transformFn()
			return
		}
		...
		const distanceNew = Math.hypot(
			firstPointer.pos.x - secondPointer.pos.x,
			firstPointer.pos.y - secondPointer.pos.y
		)
		const centerNew = {
...
function evtPointer(evt, canvasData) {
	return {
		...
		pos: evtPoint(evt),
		shift: {
			x: canvasData.position.x - evt.offsetX,
			y: canvasData.position.y - evt.offsetY,
		},
	}
}
